  ...
  return (
    <div className="p-4 border border-red-200 bg-red-50 rounded-md">
      ...
      <p className="text-xs text-red-600 mb-4">
        Компонент вызвал ошибку, но Error Boundary перехватил её и отобразил этот запасной UI вместо упавшего
        компонента.
      </p>
      <button
  ...
  return (
    <div className="p-4 border rounded-md">
      ...
      <p className="text-xs text-gray-600 mb-4">
        Этот компонент намеренно вызывает ошибку при счетчике > 3. Error Boundary перехватит ошибку и покажет запасной
        UI.
      </p>
      <div className="flex items-center space-x-4">
        <span className="text-lg font-bold">{counter}</span>
        <button onClick={handleClick} className="px-3 py-1 bg-blue-100 text-blue-700 rounded hover:bg-blue-200">
          Увеличить
        </button>
      </div>
    </div>
    ...
    <div className="max-w-3xl mx-auto">
      <div className="bg-whiteite p-6 rounded-lg shadow-md">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <div className="p-4 bg-gray-50 rounded-md">
              ...
              <p className="text-xs text-gray-600 mb-2">
                Error Boundary - это компонент React, который перехватывает ошибки JavaScript в дочернем дереве
                компонентов, логирует их и отображает запасной UI вместо упавшего дерева компонентов.
              </p>
              <p className="text-xs text-gray-600">
                В React 19 улучшена интеграция Error Boundary с Suspense и асинхронным рендерингом. Теперь ошибки в
                асинхронном коде (например, при загрузке данных) корректно перехватываются.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="p-5 bg-yellow-50 rounded-lg border border-yellow-200 mb-8">
          <h3 className="font-semibold mb-3 text-yellow-800">Улучшенные сообщения об ошибках гидратации</h3>
...
- Либо у пользователя установлено расширение, которое изменяет DOM`}
        ...
        <div className="p-5 bg-blue-50 rounded-lg border border-blue-200">
          <h3 className="font-semibold mb-3 text-blue-800">Ключевые улучшения обработки ошибок в React 19</h3>

          <div className="space-y-4">
            <div>
              <h4 className="text-sm font-semibold text-blue-700 mb-1">1. Устойчивость к внешним изменениям DOM</h4>
            ...
            <div>
              <h4 className="text-sm font-semibold text-blue-700 mb-1">2. Информативные сообщения об ошибках</h4>
              <p className="text-xs text-blue-600">
                Сообщения об ошибках стали гораздо информативнее. Теперь при несоответствии HTML React выводит одно
                подробное сообщение, в котором наглядно показывает, какой именно фрагмент не совпал, и перечисляет
                возможные причины проблемы.
              </p>
            </div>

            <div>
              <h4 className="text-sm font-semibold text-blue-700 mb-1">3. Единообразное логирование ошибок</h4>
              <p className="text-xs text-blue-600">
                React 19 избавился от двойного логирования ошибок в режиме разработки. Раньше, если ошибка была поймана
                Error Boundary, React всё равно мог вывести её стек дважды (из-за попытки повторного рендера в Strict
                Mode). Теперь таких дублей нет – ошибки выводятся единообразно.
              </p>
            </div>

            <div>
              <h4 className="text-sm font-semibold text-blue-700 mb-1">4. Улучшенная интеграция с Suspense</h4>
              <p className="text-xs text-blue-600">
                Ошибки, возникающие при загрузке данных через Suspense, теперь корректно перехватываются ближайшим Error
                Boundary. Это упрощает создание надежных асинхронных интерфейсов.
              </p>
            </div>
          </div>
        </div>
